...
impl<'f, F> AccountDeploymentV3<'f, F> {
    ...
    /// Returns a new [`AccountDeploymentV3`] with the paymaster payload.
    ...
    /// Returns a new [`AccountDeploymentV3`] with the nonce data availability mode.
    ...
    /// Returns a new [`AccountDeploymentV3`] with the fee data availability mode.
    ...
    F: AccountFactory,
{
    ...
    pub fn transaction_hash(&self, query_only: bool) -> Felt {
        ...
        hasher.update(
            (((self.inner.nonce_data_availability_mode as u64) << 32)
                + (self.inner.fee_data_availability_mode as u64))
                .into(),
        );
...
mod tests {
    ...
    #[tokio::test]
    async fn deploy_request_preserves_paymaster_fields() {
        let factory = dummy_factory();
        let request = factory
            .deploy_v3(Felt::THREE)
            .nonce(Felt::FOUR)
    ...
    #[test]
    fn deployment_hash_changes_with_paymaster_fields() {
        let factory = dummy_factory();
        let base = PreparedAccountDeploymentV3 {
            factory: &factory,
            inner: RawAccountDeploymentV3 {
                salt: Felt::THREE,
                nonce: Felt::FOUR,
        ...
        let with_paymaster = PreparedAccountDeploymentV3 {
            factory: &factory,
            inner: RawAccountDeploymentV3 {
