...
impl<'a, A> ExecutionV3<'a, A> {
    ...
    /// Returns a new [`ExecutionV3`] with the paymaster payload.
    ...
    /// Returns a new [`ExecutionV3`] with the account deployment payload.
    ...
    /// Returns a new [`ExecutionV3`] with the nonce data availability mode.
    pub fn nonce_data_availability_mode(
        self,
    ...
    /// Returns a new [`ExecutionV3`] with the fee data availability mode.
    pub fn fee_data_availability_mode(
        self,
    ...
    A: ConnectedAccount + Sync,
{
    ...
    async fn prepare(&self) -> Result<PreparedExecutionV3<'a, A>, AccountError<A::SignError>> {
        ...
        // Starknet.js signs V3 invokes with `tip ?? 0`, and Starknet's
        // transaction reference documents tip as currently always `0`.
        // Using a provider-estimated median tip here changes the signed hash
        // and diverges from the StarkZap TS path.
...
impl RawExecutionV3 {
        ...
        E: ExecutionEncoder,
    {
        ...
        hasher.update(
            (((self.nonce_data_availability_mode as u64) << 32)
                + (self.fee_data_availability_mode as u64))
                .into(),
        );
...
mod tests {
    use super::*;
    use crate::{Account, ExecutionEncoder, RawDeclarationV3};
    ...
    impl ExecutionEncoder for DummyAccount {
        fn encode_calls(&self, calls: &[Call]) -> Vec<Felt> {
            calls.iter()
                .flat_map(|call| {
                    std::iter::once(call.to)
                        .chain(std::iter::once(call.selector))
                        .chain(call.calldata.iter().copied())
                })
                .collect()
        }
    }
    ...
    impl Account for DummyAccount {
        ...
        async fn sign_execution_v3(
            &self,
            _execution: &RawExecutionV3,
            _query_only: bool,
        ) -> Result<Vec<Felt>, Self::SignError> {
            Ok(vec![Felt::ONE, Felt::TWO])
        }

        async fn sign_declaration_v3(
            &self,
            _declaration: &RawDeclarationV3,
            _query_only: bool,
        ) -> Result<Vec<Felt>, Self::SignError> {
            Ok(vec![])
        }

        fn is_signer_interactive(&self, _context: SignerInteractivityContext<'_>) -> bool {
            false
        }
    }
    ...
    #[tokio::test]
    async fn invoke_request_preserves_paymaster_fields() {
        let account = DummyAccount;
        let request = account
            .execute_v3(vec![sample_call()])
            .nonce(Felt::THREE)
            .paymaster_data(vec![Felt::ONE, Felt::TWO])
            .account_deployment_data(vec![Felt::THREE, Felt::FOUR])
        ...
        assert_eq!(request.paymaster_data, vec![Felt::ONE, Felt::TWO]);
        assert_eq!(request.account_deployment_data, vec![Felt::THREE, Felt::FOUR]);
        assert_eq!(
            request.nonce_data_availability_mode,
            DataAvailabilityMode::L2
        );
        assert_eq!(request.fee_data_availability_mode, DataAvailabilityMode::L2);
    }

    #[test]
    fn transaction_hash_changes_with_paymaster_fields() {
        let account = DummyAccount;
        let base = PreparedExecutionV3 {
            account: &account,
            inner: RawExecutionV3 {
                calls: vec![sample_call()],
                nonce: Felt::THREE,
                paymaster_data: vec![],
                account_deployment_data: vec![],
                nonce_data_availability_mode: DataAvailabilityMode::L1,
        ...
        let with_paymaster = PreparedExecutionV3 {
            account: &account,
            inner: RawExecutionV3 {
        ...
        assert_ne!(
            base.transaction_hash(false),
            with_paymaster.transaction_hash(false)
        );
    }
}
